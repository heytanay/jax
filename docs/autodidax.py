...
# Something not represented here, but present in the mainline JAX, is that
...
# `[float32[10]] -> [float32[]]`. Thus in the context of `cond`, we'd need an
# extra step of joining the two transformed jaxprs to have consistent output
# types. We don't need this step here because we chose `vmap_jaxpr`
# always to batch all outputs over the leading axis.
...
# The JVP rule introduces a wrinkle. For `jvp_jaxpr`, we have the convention
# that all the binders for tangent values are appended after all the binders for
# primal values, like `args = [*primals, *tangents]`. But that's in tension with
# our `while_loop` convention that the carry binders come after all the constant
# binders, i.e. that `args = [*consts, *carry]`, because both the constants and
# the carries can have their own tangents. For this reason, we introduce the
# `_loop_jvp_binders` helper to rearrange binders as needed.
